...
public class Trustcode extends Application {

    public static final String TAG = Trustcode.class.getSimpleName();
    ...
    public void onCreate() {
        super.onCreate();
        Trustcode.APPLICATION_NAME = getPackageManager().getApplicationLabel(getApplicationInfo()).toString();
        Trustcode.modelRegistryUtils.makeReady(getApplicationContext());
    }
    ...
    public static <T> T getModel(Context context, String modelName, OUser user) {
        Class<? extends OModel> modelCls = Trustcode.modelRegistryUtils.getModel(modelName);
        if (modelCls != null) {
            try {
